...
#include <libtorrent/write_session_params.hpp>
...
struct SessionWrapper {
    ...
    // pending resume data from save_resume_data_alert
    std::mutex resume_mu;
    std::unordered_map<int64_t, std::vector<char>> pending_resume_data;
            ...
            try {
                ...
                for (auto* a : alerts) {
                    ...
                    try {
                        ...
                        // save_resume_data_alert → queue bytes for Dart
                        else if (auto* srd = lt::alert_cast<lt::save_resume_data_alert>(a)) {
                            int64_t mid = id_for_handle(srd->handle);
                            try {
                                auto buf = lt::write_resume_data_buf(srd->params);
                                std::lock_guard<std::mutex> rlk(resume_mu);
                                pending_resume_data[mid] = std::vector<char>(buf.begin(), buf.end());
                            } catch (...) {}
                        }
...
TORRENT_API void lt_request_save_resume_data(lt_session_t session, lt_torrent_id id) {
    if (!session) return;
    auto* sw = to_sw(session);
    std::lock_guard<std::mutex> lk(sw->mu);
    auto it = sw->handles.find(id);
    if (it != sw->handles.end() && it->second.is_valid()) {
        try {
            it->second.save_resume_data(lt::torrent_handle::only_if_modified);
        } catch (...) {}
    }
}

TORRENT_API int lt_take_resume_data(lt_session_t session, lt_torrent_id id,
                                    char* out, int max_len) {
    if (!session || !out || max_len <= 0) return -1;
    auto* sw = to_sw(session);
    std::lock_guard<std::mutex> lk(sw->resume_mu);
    auto it = sw->pending_resume_data.find(id);
    if (it == sw->pending_resume_data.end() || it->second.empty()) return 0;
    int n = (int)std::min(it->second.size(), (size_t)max_len);
    std::memcpy(out, it->second.data(), n);
    it->second.erase(it->second.begin(), it->second.begin() + n);
    if (it->second.empty()) sw->pending_resume_data.erase(it);
    return n;
}

TORRENT_API lt_torrent_id lt_add_magnet_with_resume(
    lt_session_t session, const char* uri, const char* path,
    const char* resume_data, int resume_len, int stream_only) {
    if (!session || !uri || !path) { set_err("null arg"); return -1; }
    auto* sw = to_sw(session);
    try {
        lt::error_code ec;
        lt::add_torrent_params atp;
        if (resume_data && resume_len > 0) {
            lt::span<char const> span(resume_data, resume_data + resume_len);
            atp = lt::read_resume_data(span, ec);
            if (ec) { set_err(ec.message()); return -1; }
        } else {
            atp = lt::parse_magnet_uri(uri, ec);
            if (ec) { set_err(ec.message()); return -1; }
        }
        atp.save_path = path;
        atp.flags &= ~lt::torrent_flags::paused;
        ...
        if (ec) { set_err(ec.message()); return -1; }
        h.resume();
        int64_t tid = sw->next_id.fetch_add(1);
        {
            std::lock_guard<std::mutex> lk(sw->mu);
            sw->handles[tid] = h;
            if (stream_only) sw->ephemeral_torrents.insert(tid);
        }
        set_err("");
        return tid;
    } catch (const std::exception& e) { set_err(e.what()); return -1; }
...
TORRENT_API int lt_save_session_state(lt_session_t session, const char* path) {
    ...
    auto* sw = to_sw(session);
    try {
        auto params = sw->session.session_state(lt::save_state_flags_t::all());
...
TORRENT_API lt_session_t lt_create_session_from_state(
    const char* state_path, const char* listen_interface,
    int download_limit, int upload_limit) {
    try {
        lt::settings_pack sp;
        if (state_path) {
            std::ifstream ifs(state_path, std::ios_base::binary);
            if (ifs) {
        ...
        sp.set_str(lt::settings_pack::listen_interfaces,
            (listen_interface && *listen_interface) ? listen_interface : "0.0.0.0:6881,[::]:6881");
        if (download_limit > 0) sp.set_int(lt::settings_pack::download_rate_limit, download_limit);
        if (upload_limit > 0) sp.set_int(lt::settings_pack::upload_rate_limit, upload_limit);
        auto* sw = new SessionWrapper(std::move(sp));
        sw->init_default_config();
        sw->start_alert_thread();
        set_err("");
...
TORRENT_API void lt_abort_session(lt_session_t session) {
    if (!session) return;
    auto* sw = to_sw(session);
    sw->abort_requested = true;
    try { sw->session.abort(); } catch (...) {}
}
