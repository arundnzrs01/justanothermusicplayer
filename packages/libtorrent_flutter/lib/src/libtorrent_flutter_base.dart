...
class LibtorrentFlutter {
  ...
  // libtorrent alerts
  final _alertCtrl = StreamController<TorrentAlert>.broadcast();
  ...
  static Future<void> init({
    ...
    final statePath = sessionStatePath?.toNativeUtf8();
    try {
      final Pointer<LtSessionOpaque> session;
      if (statePath != null && statePath != nullptr && sessionStatePath!.isNotEmpty) {
        session = engine._b.createSessionFromState(
          statePath, iface, downloadLimit, uploadLimit,
        );
      } else {
        session = engine._b.createSession(iface, downloadLimit, uploadLimit);
      }
      if (session == nullptr) {
      ...
      malloc.free(iface);
      if (statePath != null) malloc.free(statePath);
    }
  ...
  /// Stream of libtorrent alerts (errors, finished, metadata, resume data, etc.).
  Stream<TorrentAlert> get alertStream => _alertCtrl.stream;
  ...
  int addMagnet(String magnetUri, [String? savePath, bool streamOnly = false]) {
    final m = magnetUri.toNativeUtf8();
  ...
  /// Request async resume data generation for [torrentId].
  void requestSaveResumeData(int torrentId) =>
      _b.requestSaveResumeData(_session, torrentId);

  /// Read pending resume data bytes written after [requestSaveResumeData].
  Uint8List takeResumeData(int torrentId, {int maxBytes = 512 * 1024}) {
    final buf = calloc<Char>(maxBytes);
    try {
      final n = _b.takeResumeData(_session, torrentId, buf, maxBytes);
      if (n <= 0) return Uint8List(0);
      return Uint8List.fromList(buf.cast<Uint8>().asTypedList(n));
    } finally {
      calloc.free(buf);
    }
  }

  /// Add magnet, optionally merging resume data from a prior session.
  int addMagnetWithResume(
    ...
    final m = magnetUri.toNativeUtf8();
    final s = (savePath ?? _defaultSavePath).toNativeUtf8();
    Pointer<Char> resumePtr = nullptr;
    try {
      if (resumeData != null && resumeData.isNotEmpty) {
        resumePtr = calloc<Char>(resumeData.length);
        resumePtr.cast<Uint8>().asTypedList(resumeData.length).setAll(0, resumeData);
      }
      final id = _b.addMagnetWithResume(
        _session,
        m,
        s,
        resumePtr,
      ...
      if (id >= 0) return id;
      if (!_b.hasExtendedApi && resumeData != null && resumeData.isNotEmpty) {
        return addMagnet(magnetUri, savePath, streamOnly);
      }
      if (id < 0) throw Exception(_b.lastError().toDartString());
      return id;
    } finally {
      malloc.free(m);
      malloc.free(s);
      if (resumePtr != nullptr) calloc.free(resumePtr);
    }
  }

  /// Persist libtorrent session state (DHT, etc.) to [path].
  bool saveSessionState(String path) {
    final p = path.toNativeUtf8();
    try {
      return _b.saveSessionState(_session, p) != 0;
    } finally {
      malloc.free(p);
    }
  }

  /// Begin non-blocking session shutdown.
  void abortSession() => _b.abortSession(_session);
  ...
  static void _onAlert(int type, int torrentId, Pointer<Utf8> message, Pointer<Void> userData) {
    ...
    if (inst == null || inst._alertCtrl.isClosed) return;
    final msg = message.toDartString();
    final alert = TorrentAlert.parse(type, torrentId, msg);
    inst._alertCtrl.add(alert);
  }
