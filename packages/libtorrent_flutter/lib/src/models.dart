...
/// Parsed libtorrent alert from the native engine.
enum TorrentAlertKind {
  unknown,
  finished,
  error,
  metadataReceived,
...
class TorrentAlert {
  ...
  final TorrentAlertKind kind;

  factory TorrentAlert.parse(int type, int torrentId, String message) {
    final lower = message.toLowerCase();
    TorrentAlertKind kind = TorrentAlertKind.unknown;
    if (lower.contains('finished') || lower.contains('complete')) {
      kind = TorrentAlertKind.finished;
    } else if (lower.contains('file error') || lower.contains('disk')) {
      kind = TorrentAlertKind.fileError;
    } else if (lower.contains('metadata') && lower.contains('received')) {
      kind = TorrentAlertKind.metadataReceived;
    } else if (lower.contains('save_resume_data_failed')) {
      kind = TorrentAlertKind.saveResumeDataFailed;
    } else if (lower.contains('save_resume_data') || lower.contains('resume data')) {
      kind = TorrentAlertKind.saveResumeData;
    } else if (lower.contains('error') || lower.contains('failed')) {
      kind = TorrentAlertKind.error;
    } else if (lower.contains('state update') || lower.contains('state_changed')) {
      kind = TorrentAlertKind.stateUpdate;
    } else if (lower.contains('added torrent') || lower.contains('add_torrent')) {
      kind = TorrentAlertKind.added;
    }
    return TorrentAlert(
      type: type,
      torrentId: torrentId,
  ...
  @override
  String toString() => 'TorrentAlert($kind, t=$torrentId, $message)';
}
